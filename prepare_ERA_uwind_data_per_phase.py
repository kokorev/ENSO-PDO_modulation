...
if __name__ == '__main__':
    ...
    var_fn = r'D:\data\ERA5\U-wind_850hpa\U-wind_850hpa_ens.nc'
    var = 'u'
    # read var data
    ...
    var_arr = np.array(nc.variables[var][:, 0, :, :, :])
    var_arr[var_arr < -9998] = np.nan
    var_arr = var_arr.mean(axis=1)
    ...
    for season_name, smonths, nino_sign, pdo_sign, fig_num in common.ensopdo_combinations_generator():
        ...
        try:
            with open(common.get_cache_mean_fn(var, season_name, fig_num, ymin=1981, ymax=2017), 'rb') as f:
                res_mean = pickle.load(f)
            with open(common.get_cache_mean_fn(var, season_name, fig_num, ymin=1981, ymax=2017, percents=True), 'rb') as f:
            ...
            norm = np.nanmean(select_seasonal_from_var(var_arr, dti, smonths), axis=0)
            res_prcnt = res_mean / (0.01 * norm)
            res_prcnt -= 100
            # selected_prcnt = selected_values / res_prcnt
            with open(common.get_cache_mean_fn(var, season_name, fig_num, ymin=1981, ymax=2017), 'wb') as f:
                pickle.dump(res_mean, f)
            with open(common.get_cache_mean_fn(var, season_name, fig_num, ymin=1981, ymax=2017, percents=True), 'wb') as f:
